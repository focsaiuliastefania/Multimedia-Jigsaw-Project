...
export function drawCanvas() {
    ...
    ctx.globalAlpha = 0.3;
    ctx.drawImage(puzzleImage, SOLUTION_AREA.x, SOLUTION_AREA.y, SOLUTION_AREA.width, SOLUTION_AREA.height);
    ctx.globalAlpha = 1.0;
...
function shufflePieces() {
    pieces.forEach(piece => {
        const randomX = PIECE_AREA.x + Math.random() * (PIECE_AREA.width - piece.width);
        const randomY = 50 + PIECE_AREA.y + Math.random() * (PIECE_AREA.height - piece.height - 50);
...
function onMouseDown(e) {
    ...
    for (let i = pieces.length - 1; i >= 0; i--) {
        ...
        if (mousePos.x > piece.dx && mousePos.x < piece.dx + piece.width &&
            mousePos.y > piece.dy && mousePos.y < piece.dy + piece.height) {
...
function onMouseUp(e) {
    ...
    if (Math.abs(selectedPiece.dx - selectedPiece.correctX) < snapTolerance &&
